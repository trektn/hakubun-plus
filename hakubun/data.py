...
class Data:
    ...
    # Schema version for cached per-show details (the 'extra' facts list).
    # Bump when a backend starts populating new detail fields so that
    # entries cached under the old schema get lazily re-fetched the next
    ...
    def __init__(self, messenger, config, account, mediatype):
        ...
        # Own instance attribute (not just relying on the `None` class
        # default) so get() never hands back None to a caller that runs
        # between this constructor returning and start() actually
        # populating the list -- e.g. a tracker signal firing on its own
        # thread while Engine.reload() is still mid-flight on this new
        # Data instance. An empty dict is the correct answer for "no
        # list loaded yet", not a crash.
        self.showlist = {}
    ...
    def info_get(self, show):
        ...
        if (info is None or info.get('_details_pending')
                or info.get('_info_schema') != self.info_schema):
            ...
            try:
                ...
                # Kitsu). Serve whatever the cache has -- entries written
    ...
    def info_update(self, shows):
        for show in shows:
            showid = show['id']
            # Stamp the current details schema so a later info_get() serves
            # this straight from cache instead of re-fetching (see info_get).
            show['_info_schema'] = self.info_schema
